...
public class FieldUtil {
	...
	public static EntityField getProperty(Class<?> clazz, String columnName) {
		for (EntityField field : fieldHelper.getProperties(clazz)) {
	...
	/*
	 * public static Optional<EntityField> getProperty(Class<?> clazz, String
	 * columnName) { Optional<EntityField> field =
	 * fieldHelper.getProperties(clazz).stream().filter(s ->
	 * s.getName().equals(columnName)).findFirst(); if (field.isPresent())
	...
	static class Jdk8FieldHelper implements IFieldHelper {
		...
		public List<EntityField> getFields(Class<?> entityClass) {
			/*
			 * List<EntityField> fields = List<EntityField> properties =
			 * getProperties(entityClass); Set<EntityField> usedSet = new
			 * HashSet<EntityField>(); for (EntityField field : fields) { for
			 * (EntityField property : properties) {
			 * field.setGetter(property.getGetter());
			 * field.setSetter(property.getSetter()); if
			 * (!usedSet.contains(property) &&
			 * field.getName().equals(property.getName())) {
			 * //泛型的情况下通过属性可以得到实际的类型 field.setJavaType(property.getJavaType());
			 * break; } } }
			 */
			return _getFields(entityClass, null, null);
		}
		 ...
		 * @return
		 */
		private List<EntityField> _getFields(Class<?> entityClass,
				List<EntityField> fieldList, Integer level) {
			if (fieldList == null) {
			...
			if (superClass != null && !superClass.equals(Object.class)
				...
				return _getFields(entityClass.getSuperclass(), fieldList,
						++level);
			}
			return fieldList;
		}
	...
	static class Jdk6_7FieldHelper implements IFieldHelper {
		...
		public List<EntityField> getFields(Class<?> entityClass) {
			...
			_getFields(entityClass, fieldList, _getGenericTypeMap(entityClass),
					null);
			return fieldList;
		}
		 ...
		 * @param level
		 */
		private void _getFields(Class<?> entityClass,
				List<EntityField> fieldList, Map<String, Class<?>> genericMap,
				Integer level) {
			if (fieldList == null) {
			...
			for (Field field : fields) {
				...
				if (!Modifier.isStatic(field.getModifiers())
					...
					if (field.getGenericType() != null
						...
						if (genericMap == null || !genericMap.containsKey(
								((TypeVariable) field.getGenericType())
										.getName())) {
							throw new RuntimeException(entityClass + "字段"
									+ field.getName() + "的泛型类型无法获取!");
						} else {
							entityField.setJavaType(genericMap
									.get(((TypeVariable) field.getGenericType())
											.getName()));
						}
					} else {
		...
		private Map<String, Class<?>> _getGenericTypeMap(Class<?> entityClass) {
			...
			if (superClass != null && !superClass.equals(Object.class)
				...
				if (entityClass
					...
					Type[] types = ((ParameterizedType) entityClass
							.getGenericSuperclass()).getActualTypeArguments();
					TypeVariable[] typeVariables = superClass
							.getTypeParameters();
					if (typeVariables.length > 0) {
						for (int i = 0; i < typeVariables.length; i++) {
							if (types[i] instanceof Class) {
								genericMap.put(typeVariables[i].getName(),
										(Class<?>) types[i]);
							}
						}
					}
